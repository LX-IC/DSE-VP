...
// struct GenericMMU {
struct GenericMMU : public sc_core::sc_module, public PwtModule {//libtlmpwt
    RVX_ISS &core;
    ...
    GenericMMU(sc_core::sc_module_name, RVX_ISS &core)
        : core(core), quantum_keeper(core.quantum_keeper), PwtModule(this) {//libtlmpwt
        #ifdef LIBTLMPWT
		set_activity(GenericMMU_set_activity);//libtlmpwt
		#endif
        flush_tlb();
    }
    ...
    uint64_t translate_virtual_to_physical_addr(uint64_t vaddr, MemoryAccessType type) {
        ...
        #ifdef LIBTLMPWT
		add_activity(GenericMMU_add_activity, (mmu_access_delay.value() / clock_cycle.value()));//libtlmpwt
		#endif
