...
struct GenericElfLoader : public sc_core::sc_module, public PwtModule {//libtlmpwt
	...
	GenericElfLoader(sc_core::sc_module_name, const char *filename) : filename(filename), elf(filename), PwtModule(this) {//libtlmpwt
		assert(elf.is_open() && "file not open");
		#ifdef LIBTLMPWT
		set_activity(GenericElfLoader_set_activity);//libtlmpwt
		#endif
...
//				continue;
	...
	void load_executable_image(load_if &load_if, addr_t size, addr_t offset, bool use_vaddr = true) {
		#ifdef LIBTLMPWT
		add_activity(GenericElfLoader_add_activity, GenericElfLoader_add_cycle);//libtlmpwt
		#endif
