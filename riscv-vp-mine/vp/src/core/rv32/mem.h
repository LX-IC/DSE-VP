...
struct InstrMemoryProxy : public sc_core::sc_module, public instr_memory_if, public PwtModule {//libtlmpwt
	MemoryDMI dmi;
	...
	InstrMemoryProxy(sc_core::sc_module_name, const MemoryDMI &dmi, ISS &owner) : dmi(dmi), quantum_keeper(owner.quantum_keeper), PwtModule(this) {
		#ifdef LIBTLMPWT
		set_activity(InstrMemoryProxy_set_activity);//libtlmpwt
		#endif
	}//libtlmpwt
	...
	virtual uint32_t load_instr(uint64_t pc) override {
		...
		#ifdef LIBTLMPWT
		add_activity(InstrMemoryProxy_add_activity, (access_delay.value() / clock_cycle.value()));//libtlmpwt
		#endif
...
struct CombinedMemoryInterface : public sc_core::sc_module,
	...
	// }
	CombinedMemoryInterface(sc_core::sc_module_name, ISS &owner, MMU *mmu = nullptr)
	    : iss(owner), quantum_keeper(iss.quantum_keeper), mmu(mmu), PwtModule(this) {
			#ifdef LIBTLMPWT
			set_activity(CombinedMemoryInterface_set_activity);//libtlmpwt
			#endif
	}//libtlmpwt
	...
	inline void _do_transaction(tlm::tlm_command cmd, uint64_t addr, uint8_t *data, unsigned num_bytes) {
		...
		#ifdef LIBTLMPWT
		add_activity(CombinedMemoryInterface_add_activity, (local_delay.value() / clock_cycle.value()));//libtlmpwt
		#endif
	...
	inline T _raw_load_data(uint64_t addr) {
		...
		for (auto &e : dmi_ranges) {
			if (e.contains(addr)) {
				...
				#ifdef LIBTLMPWT
				add_activity(CombinedMemoryInterface_add_activity, (dmi_access_delay.value() / clock_cycle.value()));//libtlmpwt
				#endif
