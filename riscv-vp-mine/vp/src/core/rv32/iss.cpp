...
ISS::ISS(sc_core::sc_module_name, uint32_t hart_id, bool use_E_base_isa) : systemc_name("Core-" + std::to_string(hart_id)), sm3_nice("SM3_NICE"), sm4_nice("SM4_NICE"), PwtModule(this) {//libtlmpwt
	#ifdef LIBTLMPWT
	set_activity(ISS_set_activity);//libtlmpwt
	#endif
...
void ISS::exec_step() {
	...
	// std::cout << std::hex << "data  : " << instr.data()   << std::endl;
	// std::cout << std::hex << "op    : " << op             << std::endl;
	// std::cout << std::hex << "opcode: " << instr.opcode() << std::endl;
	// std::cout << std::hex << "funct3: " << instr.funct3() << std::endl;
	// std::cout << std::hex << "funct7: " << instr.funct7() << std::endl;
			...
			switch (instr.funct3()) {
				case 0b111: {
					switch (instr.funct7()) {
						...
						case 0b0000001: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm3_nice.custom3_sm3p0(read(instr.rs1())));
							#else
							regs[instr.rd()] = sm3_nice.custom3_sm3p0(regs[instr.rs1()]);
							#endif
						} break;
						case 0b0000010: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm3_nice.custom3_sm3p1(read(instr.rs1())));
							#else
							regs[instr.rd()] = sm3_nice.custom3_sm3p1(regs[instr.rs1()]);
							#endif
						} break;
						#ifdef sm4_nice_F
						case 0b0000011: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm4_nice.custom3_sm4ks(read(instr.rs1()), read(instr.rs2())));
							#else
							regs[instr.rd()] = sm4_nice.custom3_sm4ks(regs[instr.rs1()], regs[instr.rs2()]);
							#endif
						} break;
						case 0b0000100: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm4_nice.custom3_sm4ed(read(instr.rs1()), read(instr.rs2())));
							#else
							regs[instr.rd()] = sm4_nice.custom3_sm4ed(regs[instr.rs1()], regs[instr.rs2()]);
							#endif
						} break;
						#endif
						#ifdef sm4_nice_T
						case 0b0000101: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm4_nice.custom3_sm4ks_T(read(instr.rs1())));
							#else
							regs[instr.rd()] = sm4_nice.custom3_sm4ks_T(regs[instr.rs1()]);
							#endif
						} break;
						case 0b0000110: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm4_nice.custom3_sm4ed_T(read(instr.rs1())));
							#else
							regs[instr.rd()] = sm4_nice.custom3_sm4ed_T(regs[instr.rs1()]);
							#endif
						} break;
						#endif
						#ifdef sm4_nice_L
						case 0b0000111: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm4_nice.custom3_sm4ks_L(read(instr.rs1())));
							#else
							regs[instr.rd()] = sm4_nice.custom3_sm4ks_L(regs[instr.rs1()]);
							#endif
						} break;
						case 0b0001000: {
							#ifdef PIPELINE_TIMING
							write(instr.rd(), sm4_nice.custom3_sm4ed_L(read(instr.rs1())));
							#else
							regs[instr.rd()] = sm4_nice.custom3_sm4ed_L(regs[instr.rs1()]);
							#endif
						} break;
						#endif
						#ifdef sm4_nice_l
...
void ISS::performance_and_sync_update(Opcode::Mapping executed_op) {
    ...
    #ifdef PIPELINE_TIMING
	pipeline.advance(1);
	current_cycles = pipeline.num_total_cycles;
	auto advance_cycles = current_cycles - last_cycles;
	last_cycles = current_cycles;
	sc_core::sc_time advance_time = advance_cycles * cycle_time;
	quantum_keeper.inc(advance_time);
	if (quantum_keeper.need_sync()) {
	    if (lr_sc_counter == 0) // match SystemC sync with bus unlocking in a tight LR_W/SC_W loop
		    quantum_keeper.sync();
	}
	#ifdef LIBTLMPWT
	add_activity(ISS_add_activity, advance_cycles);//libtlmpwt
	#endif
    #else
	...
	#ifdef LIBTLMPWT
	add_activity(ISS_add_activity, (new_cycles.value() / cycle_time.value()));//libtlmpwt
	#endif
	#endif
}
...
void ISS::show() {
	...
	#elif defined BRANCH_TIMING
	std::cout << "num-cycles = " << std::dec << pipeline.num_total_cycles << std::endl;
	#elif defined CACHE_TIMING
	std::cout << "num-cycles = " << std::dec << pipeline.num_total_cycles << std::endl;
	#else
	std::cout << "num-cycles = " << std::dec << _compute_and_get_current_cycles() << std::endl;
	#endif
}
